...
int f(int val_index, int num_elements, int k, int n){
	if(val_index==0){
		int num_reqd_each = k-running_sums[val_index].first;
		if(num_reqd_each+1>num_elements)
			return 0;
		int x = num_elements/(num_reqd_each+1);
		if(x>running_sums[val_index].second)
			x = running_sums[val_index].second;
		return x;	
	}
	...
	if(x>running_sums[val_index].second)
		x = running_sums[val_index].second;
	return x + f(val_index-1, num_elements-x*(num_reqd_each+1), k, n);
}
...
int query(int k, int n){
	...
	// following brute force causes TLE
	// int a = -1;
