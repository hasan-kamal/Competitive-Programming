...
int main(){
	...
	vi left_val = vi(m + 1 + 1, 0);
	vi right_val = vi(m + 1 + 1, 0);
	vi down_val = vi(n + 1 + 1, 0);
	vi up_val = vi(n + 1 + 1, 0);
	...
	cin >> count_l >> count_r >> count_t >> count_b;
	...
	for(sofa& s : sofas_fit_horizontal){
		if(s.y1 == s.y2){
			...
			if(up_val[s.y2] - 1 == count_t && down_val[s.y1] - 1 == count_b)
				ans.push_back(s);
		}
	}
